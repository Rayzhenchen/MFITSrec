...
def data_partition(fname):
    ...
    item_count = defaultdict(int)
    for line in f:
        u, i, timestamp, b, c = line.rstrip().split(' ')
        u = int(u)
        i = int(i)
        user_count[u] += 1
        item_count[i] += 1
    ...
    f.close()
    time_map = timeSlice(time_set)
    User, usernum, item_num, timenum = cleanAndsort(User, time_map)
    ...
    return [user_train, user_valid, user_test, item_meta, usernum, itemnum, timenum, bnum, cnum]


def evaluate(model, dataset, args):
    [train, valid, test, item_meta, usernum, itemnum, timenum, bnum, cnum] = copy.deepcopy(dataset)
    ...
    for u in users:
        ...
        predictions = -model.predict(*[np.array(l) for l in [[u], [seq], [time_matrix], item_idx ]], item_meta)
